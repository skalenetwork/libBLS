...
int main(int argc, char const* argv[])
{
    ...
    std::ifstream* malicious_parameters_if = new std::ifstream("parameters.json", std::ifstream::binary );
    ...
    *malicious_parameters_if >> malicious_parameters;
    ...
    subgroupPoint=libff::alt_bn128_G2(
        libff::alt_bn128_Fq2(
            libff::alt_bn128_Fq(malicious_parameters["point"]["x"][0].get<std::string>().c_str()),
            libff::alt_bn128_Fq(malicious_parameters["point"]["x"][1].get<std::string>().c_str())
        ),
        libff::alt_bn128_Fq2(
            libff::alt_bn128_Fq(malicious_parameters["point"]["y"][0].get<std::string>().c_str()),
            libff::alt_bn128_Fq(malicious_parameters["point"]["y"][1].get<std::string>().c_str())
        ),
        libff::alt_bn128_Fq2(
    ...
    malicious_dkg_wrap.setDKGSecret(std::make_shared<std::vector<libff::alt_bn128_Fr>>(malicious_polynomial));
    
    dkgs.push_back(malicious_dkg_wrap);
    
    std::shared_ptr<std::vector<libff::alt_bn128_Fr>> malicious_secret_shares = malicious_dkg_wrap.createDKGSecretShares();
    std::shared_ptr<std::vector<libff::alt_bn128_G2>> malicious_public_shares = malicious_dkg_wrap.createDKGPublicShares();
    
    // Add subgroupPoint*k_i to the i-th public share of the active adversary
    for (size_t i=0; i<num_signed; i++){
        (*malicious_public_shares)[i]=(*malicious_public_shares)[i]+malicious_polynomial[i]*subgroupPoint;
    }
    ...
    for (size_t i=0; i<num_all;i++) {
        if (i==(num_signed-1)) continue; // Skip checking active adversary's share    
    ...
    // Uncomment this to see that the check fails if adversaries start checking active adversary's shares
    /*
    ...
    */
    ...
    std::cout << "Let's check if public key is in G2: " << (libff::alt_bn128_G2::order()*(*common_public_key.getPublicKey())).is_zero() << "\n";
    ...
    for (size_t i=0; i< num_all; i++){
        ...
        BLSPublicKeyShare public_key_share=BLSPublicKeyShare(
            *private_key_share.getPrivateKey(),num_signed,num_all);
    ...
    for (size_t i=0; i<32; i++)hash_byte_arr[i]=0xff;
    ...
    for (size_t i=0; i<num_all;i++){
        if (i==num_signed-1) continue; // Active adversary doesn't have to sign, passive advesaries behave
    ...
    for (size_t i=0; i<num_all && !signature_share_set.isEnough();i++){
        signature_share_set.addSigShare(std::make_shared<BLSSigShare>(signature_shares.at(i)));
    }
