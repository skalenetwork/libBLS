...
BOOST_AUTO_TEST_SUITE(Bls)
        ...
        for (size_t i = 0; i < 10; ++i) {
            ...
            for (size_t i = 0; i < 10; ++i) {
                ...
                for (size_t i = 0; i < num_signed; ++i) {
                    ...
                    BOOST_REQUIRE_THROW(BLSSigShare(bad_sig, hint, participants.at(i), num_signed, num_all), signatures::Bls::IsNotWellFormed);
                }
    ...
    BOOST_AUTO_TEST_CASE(BLSWITHDKG) {
      ...
      for (size_t i = 0; i < 10; ++i) {
        ...
        for ( size_t i = 0; i < num_all; i++){
          BLSPrivateKeyShare pkey_share =  dkgs.at(i).CreateBLSPrivateKeyShare(std::make_shared<std::vector<libff::alt_bn128_Fr>>(secret_key_shares.at(i)));
    ...
    BOOST_AUTO_TEST_CASE(Exceptions) {
      ...
      {
        BOOST_REQUIRE_THROW(BLSPrivateKey skey(std::make_shared<std::string>("0"), num_signed, num_all ), signatures::Bls::ZeroSecretKey);
      }

      {
        BOOST_REQUIRE_THROW(BLSPrivateKey skey(nullptr, std::make_shared<std::vector<size_t>>(participants), num_signed, num_all ), signatures::Bls::IncorrectInput);
      }

      {
        BOOST_REQUIRE_THROW(BLSPrivateKey skey(BLSPrivateKeyShare::generateSampleKeys(num_signed, num_all)->first, NULL, num_signed, num_all), signatures::Bls::IncorrectInput);
      }

      {
        BOOST_REQUIRE_THROW(BLSPrivateKeyShare skey("", num_signed, num_all ), signatures::Bls::IncorrectInput);
      }

      {
        BOOST_REQUIRE_THROW(BLSPrivateKeyShare skey("0", num_signed, num_all ), signatures::Bls::ZeroSecretKey);
      }

      {
        BOOST_REQUIRE_THROW(BLSPrivateKeyShare skey(libff::alt_bn128_Fr::zero(), num_signed, num_all ), signatures::Bls::ZeroSecretKey);
      }
      ...
      {
        BLSPrivateKeyShare skey(libff::alt_bn128_Fr::random_element(), num_signed, num_all ); // Zero signer index
        BOOST_REQUIRE_THROW(skey.signWithHelper(std::make_shared<std::array<uint8_t, 32>>(GenerateRandHash()), 0), signatures::Bls::IncorrectInput);
      }
...

      {
        ...
        BOOST_REQUIRE_THROW(BLSPublicKey pkey(null_vect, num_signed, num_all), signatures::Bls::IncorrectInput);
      }

      {
        BOOST_REQUIRE_THROW(BLSPublicKey pkey(libff::alt_bn128_G2::zero(), num_signed, num_all), signatures::Bls::IsNotWellFormed);
      }

      {
        BOOST_REQUIRE_THROW(BLSPublicKey pkey(libff::alt_bn128_Fr::zero(), num_signed, num_all), signatures::Bls::IsNotWellFormed);
      }
      ...
      is_exception_caught = false;   // Null signature in Verify Signature
      {
        ...
        BOOST_REQUIRE_THROW(pkey.VerifySigWithHelper(std::make_shared<std::array<uint8_t, 32> >(GenerateRandHash()), nullptr, num_signed,
                                 num_all), signatures::Bls::IncorrectInput);
      }

      {
        BOOST_REQUIRE_THROW(BLSPublicKeyShare pkey(libff::alt_bn128_Fr::zero(), num_signed, num_all), signatures::Bls::ZeroSecretKey);
      }

      {
        BOOST_REQUIRE_THROW(BLSPublicKeyShare pkey(nullptr, num_signed, num_all), signatures::Bls::IncorrectInput);
      }

      {
        ...
        BOOST_REQUIRE_THROW(pkey.VerifySigWithHelper(nullptr, std::make_shared<BLSSigShare>(rand_sig), num_signed, num_all), signatures::Bls::IncorrectInput);
      }
      ...
      {
        BOOST_REQUIRE_THROW(BLSSignature(nullptr, num_signed, num_all), signatures::Bls::IncorrectInput);
      }

      {
        std::string hint = "123:1";
        BOOST_REQUIRE_THROW(BLSSignature(nullptr, hint, num_signed, num_all), signatures::Bls::IncorrectInput);
      }

      {
        std::string empty_hint = "";
        BOOST_REQUIRE_THROW(BLSSignature(std::make_shared< libff::alt_bn128_G1 >(libff::alt_bn128_G1::random_element()), empty_hint, num_signed, num_all), signatures::Bls::IncorrectInput);
      }

      {
        std::string short_sig = "1:1:1:1";
        BOOST_REQUIRE_THROW(BLSSignature(std::make_shared< std::string >(short_sig), num_signed, num_all), signatures::Bls::IsNotWellFormed);
      }

      {
        ...
        BOOST_REQUIRE_THROW(BLSSignature(std::make_shared< std::string >(long_sig), num_signed, num_all), signatures::Bls::IsNotWellFormed);
      }

      {
        ...
        BOOST_REQUIRE_THROW(BLSSignature(std::make_shared< std::string >(long_sig), num_signed, num_all), signatures::Bls::IncorrectInput);
      }
      ...
      {
        BOOST_REQUIRE_THROW(BLSSigShare(nullptr, 1, num_signed, num_all), signatures::Bls::IncorrectInput);
      }

      {
        std::string hint = "123:1";
        BOOST_REQUIRE_THROW(BLSSigShare( std::make_shared<libff::alt_bn128_G1>(libff::alt_bn128_G1::random_element()), hint, 0, num_signed, num_all), signatures::Bls::IncorrectInput);
      }

      {
        std::string empty_hint = "";
        BOOST_REQUIRE_THROW(BLSSigShare(std::make_shared< libff::alt_bn128_G1 >(libff::alt_bn128_G1::random_element()), empty_hint, 1, num_signed, num_all), signatures::Bls::IncorrectInput);
      }

      {
        std::string short_sig = "1:1:1:1";
        BOOST_REQUIRE_THROW(BLSSigShare(std::make_shared< std::string >(short_sig), 1, num_signed, num_all), signatures::Bls::IsNotWellFormed);
      }
...

      {
        ...
        BOOST_REQUIRE_THROW(BLSSigShare(std::make_shared< std::string >(long_sig), 1, num_signed, num_all), signatures::Bls::IncorrectInput);
      }
