...
BOOST_AUTO_TEST_CASE( ThresholdEncryptionWrappers ) {
    ...
    for (size_t i = 0; i < numSigned; ++i) participants.at(i) = i + 1; //set participants indices 1,2,3
    ...
    // print results
    print_args( args );
    std::cout << "Average partial signature time: " << ( partialSignature / args.rounds ) << " ms\n";
    std::cout << "Average signature merge time: " << ( signatureMerge / args.rounds ) << " ms\n";
    std::cout << "Average adding signature share time: " << ( addingSigShare / args.rounds ) << " ms\n";
