...
std::vector<uint8_t>
ThresholdEncryption::mockupEncrypt(const std::vector<uint8_t>& _message) {
    if (_message.empty()) {
    ...
    message_to_cipher.insert(
        message_to_cipher.end(), random_secret.begin(), random_secret.end());
...
std::vector<uint8_t>
ThresholdEncryption::mockupDecrypt(const std::vector<uint8_t>& _encrypteData) {
